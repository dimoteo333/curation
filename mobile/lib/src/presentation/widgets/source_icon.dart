...
class _SourceIconPainter extends CustomPainter {
  ...
  bool get _isDiary => source == 'diary' || source == 'journal' || source == 'file';
  bool get _isCalendar => source == 'calendar';
  bool get _isMemo => source == 'memo' || source == 'note';
  bool get _isVoice => source == 'voice_memo' || source == 'voice' || source == 'audio';
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (_isDiary) {
      final rect = RRect.fromRectAndRadius(
        Rect.fromLTWH(size.width * 0.18, size.height * 0.14, size.width * 0.64, size.height * 0.72),
    ...
    if (_isCalendar) {
      final rect = RRect.fromRectAndRadius(
        Rect.fromLTWH(size.width * 0.16, size.height * 0.22, size.width * 0.68, size.height * 0.56),
    ...
    if (_isVoice) {
      final rect = RRect.fromRectAndRadius(
        Rect.fromLTWH(size.width * 0.38, size.height * 0.14, size.width * 0.24, size.height * 0.42),
        Radius.circular(size.width * 0.14),
      );
      canvas.drawRRect(rect, stroke);
      canvas.drawArc(
        Rect.fromCircle(center: Offset(size.width * 0.5, size.height * 0.46), radius: size.width * 0.24),
        math.pi,
        math.pi,
        false,
        stroke,
    ...
    canvas.drawCircle(Offset(size.width * 0.5, size.height * 0.5), size.width * 0.22, stroke);
  }
