...
| 6. 저장 데이터 보호 부재 | 문서 검증으로 보안 체크리스트 유지 | SQLite 암호화 도입 계획 필수 | 평문 저장이 남아 있음을 숨기지 않고, 암호화 도입 전제조건을 설계 문서에 기록한다. |
...
1. Phase 0: 현재 평문 저장을 문서와 개발자 패널에 명시한다.
2. Phase 1: 새 설치에는 SQLCipher 기반 암호화 DB를 사용하고, 기존 평문 DB는 읽기 전용 마이그레이션 경로를 둔다.
3. Phase 2: 앱 시작 시 키체인/Keystore 기반 키를 확보하고, 평문 DB를 암호화 DB로 복제한 뒤 원본을 안전 삭제한다.
4. Phase 3: 원격 백업/복구 정책이 생기면 키 회전과 복구 불가 시 UX를 별도 설계한다.
