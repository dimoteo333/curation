...
- 모델 경로 누락 확인:
  `--dart-define=LLM_MODEL_PATH=` 와 `--dart-define=EMBEDDER_MODEL_PATH=` 상태에서 홈 화면 런타임 배지가 템플릿 폴백으로 표시되어야 한다.
- 잘못된 경로 확인:
  존재하지 않는 절대 경로를 넘겨 초기화 실패 메시지와 개발자 패널 상세 정보 노출을 확인한다.
- 네이티브 확인:
  실제 모델 파일을 연결한 뒤 홈 화면이 `네이티브 LLM 사용 중`으로 표시되고, 생성 결과가 폴백 문구 없이 반환되는지 본다.
- 회귀 확인:
  네이티브 실패 시에도 질문 제출, 검색 결과, 후속 질문 UX가 유지되어야 한다.
...
- 모델 파일과 로컬 DB는 평문 저장을 기본값으로 두지 않는 방향으로 설계하고, 민감 데이터는 암호화 도입 경로를 함께 기록한다.
